...
output_dir = 'Chap2/sortie'
if not os.path.exists(output_dir):
    os.makedirs(output_dir)


img1 = Image.open('entree/henri1.png').convert('L')
img2 = Image.open('entree/henri2.png').convert('L')
...
plt.title('Addition (henri1 + henri2)')
plt.axis('off')

plt.subplot(2, 3, 5)
plt.imshow(sub_result, cmap='gray')
plt.title('Subtraction (henri1 - henri2)')
plt.axis('off')

plt.subplot(2, 3, 6)
plt.imshow(mult_result, cmap='gray')
plt.title(f'Multiplication (henri1 * {ratio})')
plt.axis('off')
